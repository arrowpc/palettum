...
import PaletteEditor from "@/components/palette/palette-editor/palette-editor";
...
const PaletteListItem = React.memo(function PaletteListItem({
  palette,
  isSelected,
  isHovered,
  startIndex,
  ...
  onShowMobileMenu: (id: string) => void;
}) {
  return (
    <div
      className={cn(
      ...
      onMouseEnter={() => onHover(palette.id)}
    ...
    >
      <div className="flex items-center min-w-0">
        ...
        {palette.source && (
          <TooltipWrapper enabled={isHovered} content="View source">
            <a
              href={palette.source}
              target="_blank"
      ...
      <div className="hidden sm:flex items-center justify-end gap-1">
        <TooltipWrapper enabled={isHovered} content="Duplicate">
          <button
            onClick={(e) => {
              e.stopPropagation();
              onCopy(palette);
        ...
        <TooltipWrapper enabled={isHovered} content="Export">
          <button
            onClick={(e) => {
              e.stopPropagation();
              onExport(palette);
        ...
        <TooltipWrapper enabled={isHovered} content="Edit">
          <button
            onClick={(e) => {
              e.stopPropagation();
              if (palette.kind === "Custom") onEdit(palette);
            }}
            className={cn(
              "p-1.5",
              palette.kind === "Default"
        ...
        <TooltipWrapper enabled={isHovered} content="Delete">
          <button
            onClick={(e) => {
              e.stopPropagation();
              if (palette.kind === "Custom") onDelete(palette.id);
            }}
            className={cn(
              "p-1.5",
              palette.kind === "Default"
...
function PaletteManager() {
  ...
  const handleCopyPalette = useCallback(
    ...
    [palettes, addPalette, setSelectedPalette],
  );

  const handleExportPalette = useCallback((palette: Palette) => {
      ...
      if (file) {
        ...
        reader.onload = (e: ProgressEvent<FileReader>) => {
          try {
            ...
            alert(
              `Failed to import palette: ${error instanceof Error ? error.message : "Unknown error"
  ...
  const handleSavePalette = useCallback(
    async (updatedPalette: Palette) => {
      const originalId = editingPalette?.id;
      if (originalId) {
  ...
  const handleHoverOn = useCallback((id: string) => {
    setHoveredPaletteId(id);
  }, []);
  ...
  const handleShowMobileMenu = useCallback((id: string) => {
    setMobileActionPaletteId(id);
  }, []);
              ...
              {sortedAndFilteredPalettes.map((palette) => {
                ...
                return (
                  <PaletteListItem
                    ...
                    isHovered={palette.id === hoveredPaletteId}
                    startIndex={startIndex}
                    onSelect={handleSelectPalette}
                    onHover={handleHoverOn}
                    onLeave={handleHoverOff}
